# Portfolios of Joo Yong Gyu

응용 프로그램 및 백엔드를 중점으로 경험한 개발자 주용규입니다.

# Work Experience

# Freelancer

## 우송대학교&충남대학교 자동청력검사기 개발

우송대학교 의뢰 : 국산 청력검사기 개발

데시벨&주파수 조정 기능 추가

수정상승법에 의거한 자동 청력 검사 기능 추가

기타 UI 확정 및 외관 제작 협의

충남대학교 의뢰 : 우송대학교에서 진행한 자동청력검사기의 추가 보완 및 개선

수정상승법에 의거한 자동청력 검사 기능 로직 개선

순음 음질 개선

기타 UI 변경 작업

제품 외관 변경 등

## 수행 기간

2023.10 ~ 2024.01

## 기술 스택

Python(tkinter), Raspberry Pi, Linux

## 참고 사진

![자동검사_시나리오 검사 결과 영문](https://github.com/Turtlebacker/portfolios/assets/97995992/e9a78eae-74b7-40dd-b305-ecc6467a3742)

![청력검사기_표지](https://github.com/Turtlebacker/portfolios/assets/97995992/3154fd41-3911-4be3-98ca-19f23b94dee5)



## 방탈출 카페 콘텐츠 개발 용역

방탈출 카페에 사용될 콘텐츠 개발

미로찾기 게임 제작
- 터치 기능 추가
- 아두이노 연동 추가

비밀번호 입력 게임 제작
- 아두이노 연동 추가
- alt+F4 강제 종료 비활성화 기능 추가

화면 부수기 게임 제작
- 아두이노 연동 추가
- 시리얼 통신 기능 추가


## 수행 기간

2023.11 ~ 2024.01

## 기술 스택

Java, Processing, Arduino, C/C++

## 참고 사진

![스크린샷 2024-02-29 174214](https://github.com/Turtlebacker/portfolios/assets/97995992/5ae9346f-a0c1-4ebe-a860-5a51c35cee08)

![스크린샷 2024-02-29 174323](https://github.com/Turtlebacker/portfolios/assets/97995992/4f86a9f1-b062-408c-8758-0a01eb8a9ef4)




# 주식회사 아이티존
  2022.08 ~ 2024.01

## 드론 대응 기술 적용을 위한 지능형 학습 모델 개발

드론 대응 기술 적용을 위한 지능형 학습 모델 개발

가상 학습용 시험 데이터셋 선정

가상 학습용 시험 데이터셋 자동 생성 모듈 개발


## 수행 기간

2023.10 ~ 2023.12

## 기술 스택

Python

## 참고 사진

보안 문제로 비공개


## 사이버위협정보지식그래프데이터연동도구및분석관리ui 플랫폼 유지보수

사이버 위협 정보 지식 그래프 데이터 연동 도구의 수집 모듈 관리

수집 데이터 관리 및 이상징후 관리

수집 모듈 로직 개선 및 문제 점검


## 수행 기간

2023.06 ~ 2024.01

## 기술 스택

Python, Hadoop, Solr, Neo4j, Ambari

## 참고 사진

보안 문제로 비공개


## 국가기간망 침투 대응 훈련 시스템 및 콘텐츠 유지보수

국가기간망 침투 대응 훈련 콘텐츠 개발
- Log4J 취약점(CVE-44228)
- Victim server, computer 납품
- 훈련 환경은 VM(가상머신)으로 제작
- 로그 보존을 위한 초기화 스크립트 개발

훈련 환경 배포를 위한 가상머신 배포 스크립트 개발

훈련 문제 출제
- server 약 40문항, computer 약 40문항
  

## 수행 기간

2022.08 ~ 2023.06

## 기술 스택

Java, springBoot, VM, CentOS, PowerShellScripts, VMware, Vsphere, ShellScripts, GNS3

## 참고 사진

![109](https://github.com/Turtlebacker/portfolios/assets/97995992/49d87371-f9bb-4688-a2f7-93d5e766e39c)



# 메이킹협동조합
  2020.12 ~ 2022.06

## 국가과제 ICT : SOS랩을 활용한 SW서비스 개발 – 코로나바이러스 노출 위험 감소를 위한 손 씻기·건조 시스템 개발

손 씻기/건조 시스템 : 손 씻는 행동을 안내하는 콘텐츠 프로그램 개발 현황 정리 및 관리

손 씻기 앱과 연동하기 위한 API 구축


## 수행 기간

2021.06 ~ 2021.12

## 기술 스택

JavaScripts, GraphQL, express-graphql

## 참고 사진

보안 문제로 비공개개

## 2020 시민참여형 사회문제해결 ICT서비스 기술개발 지원사업

복지관에서 사용 가능한 스케쥴 알람 프로그램 개발


## 수행 기간

2020.12 ~ 2021.02

## 기술 스택

C/C++, MFC

## 참고 사진

![캡처1](https://github.com/Turtlebacker/portfolios/assets/97995992/60e825e5-8c0c-4c80-9317-6069f1aae8ed)

![캡처2](https://github.com/Turtlebacker/portfolios/assets/97995992/a73e8741-b46c-4eca-9f0b-0c14b216768f)


